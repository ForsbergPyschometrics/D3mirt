...
To solve a mathematical word problem, one must often have both verbal and mathematical skills, refereed to as abilities (denoted $\theta$) in the literature on IRT.
...
i.e., a type of measurement model within multidimensional IRT that uses linear combinations of $\theta$-values for ability assessment.
Accordingly, in this type of model it assumed that the same probability score for a correct response can be reached by adding different combinations of $\theta$-values. In turn, this implies that the compensatory model allows items to be unidimensional, i.e., that they measures a single ability, or *within-multidimensional*, i.e., that the items measures more than one ability in the model space.

Note, the `D3mirt` approach is limited to two types of compensatory models, dependent on item type.
...
Compared to other software, `D3mirt` is unique in implementing DMIRT methodology explicitly and comprehensively in a three-dimensional interactive environment. For instance, the compensatory model can be fitted with software dedicated to multidimensional IRT that supports dichotomous or polytomous data and that allows the user to specify the necessary factor structure. This includes software such as `Mplus` [@MPLUS] or `IRTPRO 2.1` [@IRTPRO] and `flexMIRT` [@flexMIRT] for the Windows environment. Using these software programs, the DMIRT estimates can then be derived manually with the help of general mathematical software, such as `MATLAB` [@MATLAB] or `Mathematica` [@Mathematica], and plotted with built-in options for creating vector plots in two or three-dimensions. This is, however, often time-consuming, and the plotting methods are not optimized for DMIRT analysis and for test development. Regarding `R`, the `mirt` package [@Chalmers:2012] can be used to fit the compensatory multidimensional model and derive the basic DMIRT item and person parameters whuile the vector plot options are limited. There is also the `R` package `plink` [@Weeks:2010] that offers two-dimensional vector plots suitable for DMIRT analysis but only for dichotomous items. Another more general limitation is that none of the formerly mentioned software applications offers a function that can help identify the DMIRT model. The user is, therefore, left to a process of trial and error. 
