...
A new addition in version 2.0.0 of `D3mirt`\`is the option of fitting an orthogonal constrained version of the compensatory model. This model allows the user to restrict items in the model space to only load on one of the model axes respectively. This allows for investigating of the items under the assumption that they are unidimensional in the three-dimensional space. The term *orthogonal*, in this context, refers to the perpendicular orientation of the three unidimensions in the model space.

To tell `D3mirt` to use the orthogonal model, create a nested list of items specifying the with items load to which dimension. In the example below, items 1 to 10 will be constrained to the $x$-axis, items 15 to 19 to the $y$-axis, and items 11 to 14 to the $z$-axis.
...
It is also possible to plot a subset of respondents in the model space. To this the user must first create an data frame with respondent trait scores externally and use the same in the `profiles`argument in `plot()`.

### 3.2.2 Plotting the Orthogonal model

The orthogonal model reduces the effect on gender seen in the compensatory model.
```{r, testg11, webgl=TRUE, fig.width = 7, fig.height = 7}
# Plot profiles with item vector arrows hidden
# Score levels: 1 = Blue ("male") and 2 = Red ("female")
plot(mod2, hide = FALSE, ind.scores = TRUE, 
     levels = x[, 2],
     sphere.col = c("blue", "red"), 
           x.lab = "Compassion", 
           y.lab="Conformity", 
...
Below the model syntax for the compensatory model and mirt code used in `D3mirt` can be seen. This is the same syntax frame that is built in to `D3mirt` version $\geq$ 2.0.0. In brief, the three-dimensional compensatory model is specified so that all items load on all three factors in the model, and all factors are constrained to be orthogonal. Fitting the model is preferably done with the `mirt()` function from the package `mirt` (Chalmers, 2012).
