...
A new addition in version 2.0.0 of `D3mirt` is the option of fitting an orthogonal constrained version of the compensatory model. This model allows the user to restrict items in the model space to only load on one of the model axes respectively. The orthogonal model is foremost useful for studying the items under the assumption that the items are unidimensional and that the unidimensions are uncorrelated. The term *orthogonal*, in this context, refers to the geographical interpretation of the graphical output in which all three unidimensions are perpendicular in the model space.
