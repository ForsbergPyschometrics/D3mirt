...
#' @param con.pol Optional. Nested lists of spherical angles to identify constructs. The default is `con.pol = NULL`.
...
#' }
#' @export
D2mirt <- function(x, modid = NULL, model = NULL, con.items = NULL, con.pol = NULL, itemtype = "graded", method.mirt = "QMCEM", method.fscores = "EAP", QMC = TRUE){
  ...
  if (isS4(x)){
    ...
    if(!is.null(modid)){
      if (length(modid) > 1) stop("The model identification argument contains too many elements")
      mod <- NULL
  ...
  if (ncol(x) < 4) stop("The data frame must have at least 3 columns")
  a <- x[, 1:2, drop = FALSE]
  ndiff <- ncol(x)-2
  diff <- x[, (3):(2+ndiff), drop = FALSE]
  mdisc <- sqrt(rowSums(a^2))
  md <- mdisc%*%matrix(rep(1,2), nrow=1, ncol=2)
  dcos <- as.matrix(a/md, ncol = 2)
  theta <- NULL
  ...
  for (i in seq_len(ndiff)){
    ...
    vector1 <- as.matrix(rbind(vector1,vec1), ncol = 2)
    vector2 <- as.matrix(rbind(vector2,vec2), ncol = 2)
  ...
  if (!is.null(con.items)){
    ...
    for (i in seq_along(con.items)){
      ...
      for (i in seq_along(l)){
        n <- l[i]
        m <- dcos[n,]
        cosk <- as.matrix(rbind(cosk,m), ncol = 2)
      }
      cscos <- matrix(colSums(cosk), ncol = 2)
      cdcos <- 1/sqrt(rowSums(cscos^2))*cscos
      maxnorm <- (1.1*max(vector1))*cdcos
      minnorm <- (0.6*min(vector1))*cdcos
      con <- as.matrix(rbind(con,rbind(minnorm, maxnorm)), ncol = 2)
      ncos <- as.matrix(rbind(ncos,cdcos), ncol = 2)
      theta <- NULL
      ...
      phi <- 90
      pol <- cbind(theta, phi)
      cpol <- as.matrix(rbind(cpol,pol), ncol = 2)
      disc <-  apply(a, 1, function(x) x %*% t(cdcos))
      ddisc <- as.matrix(cbind(ddisc, disc), ncol = 1)
    }
  }
  if (!is.null(con.pol)){
    if(!is.list(con.pol)) stop("The speherical coordinates for constructs must be of type list")
    con <- NULL
    cpol <- NULL
    ncos <- NULL
    cdcos <- NULL
    ...
    for (i in seq_along(con.pol)){
      ...
      j <- cos(r)
      k <- sin(r)
      cdcos <- as.matrix(cbind(j,m), ncol = 2)
      maxnorm <- (1.1*max(vector1))*cdcos
      minnorm <- (0.6*min(vector1))*cdcos
  ...
  colnames(dcos) <- c("D.Cos X", "D.Cos Y")
  ...
  if (!is.null(con.items) || !is.null(con.pol)){
    ...
    colnames(ncos) <- c("C.Cos X","C.Cos Y")
  ...
  if (!is.null(con.items)){
    D2mirt <- list(loadings = a, modid = modid, diff = diff, mdisc = mdisc, mdiff = mdiff, dir.cos = dcos, polar = pol, c.dir.cos = ncos , c.pol = cpol, ddisc = ddisc,
                   dir.vec = dir.vec, scal.vec = scal.vec, con.items = con.items,  c.vec = con, fscores = trait)
  } else if (!is.null(con.pol)){
    D2mirt <- list(loadings = a, modid = modid, diff = diff, mdisc = mdisc, mdiff = mdiff, dir.cos = dcos, polar = pol, c.dir.cos = ncos , c.pol = cpol, ddisc = ddisc,
                   dir.vec = dir.vec, scal.vec = scal.vec, con.pol = con.pol,  c.vec = con, fscores = trait)
  } else if (!is.null(trait)) {
    D2mirt <- list(loadings = a, modid = modid, diff = diff, mdisc = mdisc, mdiff = mdiff, dir.cos = dcos, polar = pol, diff = diff,
